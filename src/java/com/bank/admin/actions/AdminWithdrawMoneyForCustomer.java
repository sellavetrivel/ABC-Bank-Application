 ...
 */
public class AdminWithdrawMoneyForCustomer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String admin = s.getAttribute("admin").toString();

            try {
                DbConnection dbb = new DbConnection();
                con = dbb.getConnection();
                ps = con.prepareStatement(DbQueries.ADMIN_SELECT_EXSISTING_USER_ACCOUNT);
                ...
                if (dbamo >= useramo) {
                    ...
                    ps = con.prepareStatement(DbQueries.ADMIN_UPDATE_WITHDRAW);
                    ps.setFloat(1, reduce);
                    ps.setString(2, accno);
                    ps.setString(3, ifsc);
                    int y = ps.executeUpdate();
                    if (y != 0) {
                        ps = con.prepareStatement(DbQueries.ADMIN_WITHDRAW_FOR_USER);
                        ps.setString(1, accno);
                        ps.setString(2, ifsc);
                        ...
                        out.println("location='AdminWithdrawForUser.jsp'");
                        out.println("</script>");
                    }
